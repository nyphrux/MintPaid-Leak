...
using static StupidTemplate.ThemeSettings;
...
namespace StupidTemplate.Menu
{
    ...
    public class Main : MonoBehaviour
    {
        ...
        private const string menuOpenSoundUrl = "https://github.com/iiDk-the-actual/ModInfo/raw/main/open.wav"; 
        private static AudioClip cachedMenuClip;

        public static float num = 7f;
        private static async Task<AudioClip> LoadSoundFromURLAsync(string url)
        {
            using (UnityWebRequest unityWebRequest = UnityWebRequestMultimedia.GetAudioClip(url, AudioType.WAV))
            {
                ...
                {
                    Debug.LogError("Failed to load audio because: " + unityWebRequest.error);
                    return null;
                }
        ...
        public static bool spinmenuz;
        public static bool spinmenux;
        public static bool spinmenuy;
        public static bool orbitEnabled;
        public static bool floatEnabled;
        ...
        private Coroutine themeUpdateCoroutine;
        private static IEnumerator PlayDelayed(AudioClip clip, float delay)
        {
        ...
        public static async void Prefix()
        {
...

                {
                    ...
                    if (menu == null)
                    {
                        if (toOpen || keyboardOpen)
                        {
                            ...
                            ThemeChecker.CheckAndUpdateTheme();
                            ...
                            if (cachedMenuClip == null)
                                cachedMenuClip = await LoadSoundFromURLAsync(menuOpenSoundUrl);

                            if (cachedMenuClip != null)
                            {
                                
                                audioSource.PlayOneShot(cachedMenuClip, 1f);


                                audioSource.PlayOneShot(cachedMenuClip, 1f);
                                CoroutineHandler.Instance.StartCoroutine(PlayDelayed(cachedMenuClip, 0.01f));
                            }
                        }
                    ...
                    else
                    {
                        ...
                        else
                        {
                            ...
                            if (!floatEnabled && !spinmenuy && !spinmenux && !orbitEnabled && !Trailmenu && !spinmenuz)
                            {
                                Rigidbody comp = menu.AddComponent<Rigidbody>();
                                comp.velocity = rightHanded
                                    ? GorillaLocomotion.GTPlayer.Instance.rightHandCenterVelocityTracker.GetAverageVelocity(true, 0)
                                    : GorillaLocomotion.GTPlayer.Instance.leftHandCenterVelocityTracker.GetAverageVelocity(true, 0);
                            ...
                            {
                                if (spinmenuz) MenuPhysicsMods.SpinMenuz();
                                if (spinmenuy) MenuPhysicsMods.Spinmenuy();
                                if (Trailmenu) MenuPhysicsMods.InitMenuTrail();
                                if (spinmenux) MenuPhysicsMods.Spinmenux();

                                if (orbitEnabled) MenuPhysicsMods.OrbitMenu();
                                if (MenuPhysicsMods.menuRb != null)
                                {
                                    MenuPhysicsMods.menuRb.velocity = rightHanded
                                        ? GorillaLocomotion.GTPlayer.Instance.rightHandCenterVelocityTracker.GetAverageVelocity(true, 0)
                                        : GorillaLocomotion.GTPlayer.Instance.leftHandCenterVelocityTracker.GetAverageVelocity(true, 0);
                                }
                            }
            ...
            {
                UnityEngine.Debug.LogError($"{PluginInfo.Name} // Error initializing at {exc.StackTrace}: {exc.Message}");
            }
            ...
            try
            {
                ...
                foreach (ButtonInfo[] buttonlist in Players)
                        {
        ...
        {
            string targetText = "Mint Paid";
            string bluePart = "Mint";

            int totalLength = targetText.Length;

            while (true)
            {
                for (int visibleLetters = 0; visibleLetters <= totalLength; visibleLetters++)
                {
                    ...
                    for (int i = 0; i < visibleLetters; i++)
                    {
                        ...
                        if (i < bluePart.Length)
                        {
                            // Blue letters
                            display.Append($"<color=#00BFFF>{c}</color>");
                        }
                        else
                        {
                            // White letters
    ...
    ButtonCornerD
            ...
            colorChanger = gameObject.AddComponent<ColorChanger>();
                    ...
                    ButtonInfo[] activeButtons = Players[buttonsType].Skip(pageNumber * buttonsPerPage).Take(buttonsPerPage).ToArray();
    ...
    RoundCornerD
        ...
        {
            int lastPage = ((Players[buttonsType].Length + buttonsPerPage - 1) / buttonsPerPage) - 1;
        ...
        {
            foreach (ButtonInfo[] buttons in Players)
            {
