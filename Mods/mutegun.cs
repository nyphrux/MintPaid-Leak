...
namespace StupidTemplate.Menu
{
    class mutegun
    {
        ...
        private static float delayformute = 0f;
        ...
        public static void Mutegun()
        {
            if (ControllerInputPoller.instance.rightControllerGripFloat > 0.1f || UnityInput.Current.GetMouseButton(1))
            {
                if (Physics.Raycast(GorillaLocomotion.GTPlayer.Instance.rightControllerTransform.position, -GorillaLocomotion.GTPlayer.Instance.rightControllerTransform.up, out var hitinfo))
                {
                    ...
                    {
                        // Try to get the VRRig from the hit object
                        VRRig hitRig = hitinfo.transform.GetComponentInParent<VRRig>();
                        if (hitRig != null)
                        {
                            Photon.Realtime.Player hitPlayer = RigManager.GetPlayerFromVRRig(hitRig);
                            if (hitPlayer != null)
                            {
                                foreach (GorillaPlayerScoreboardLine line in GorillaScoreboardTotalUpdater.allScoreboardLines)
                                {
                                    if (line.linePlayer != null && line.linePlayer.UserId == hitPlayer.UserId)
                                    {
                                        delayformute = Time.time + 0.5f;
